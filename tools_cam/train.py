...
def future_loss(pred, target):
    pred_cal = pred.softmax(dim=1)
    pred_enp = -torch.log2(pred_cal)
    pred_enp = pred_enp * pred_cal
    pred_enp = torch.sum(pred_enp,dim=1) / 7
    loss = torch.where(target == 1, -torch.log(1 - pred_enp), -torch.log(pred_enp))
    loss = loss.mean()
    return loss

def single_cls_loss(pred, target):
...
def creat_model(cfg, args):
    ...
    print(model)
   # checkpoint = torch.load('./checkpoints/bird_tiny.pth')
...
def main():
    ...
    for epoch in range(1, cfg.SOLVER.NUM_EPOCHS+1):
        ...
        if epoch >= cfg.SOLVER.NUM_EPOCHS-10:
            update_val_step, loss_val = \
...
def train_one_epoch(train_loader, model, device, criterion, optimizer, epoch,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        loss_single_point = single_cls_loss(cams, target)
        loss = criterion(cls_logits, target) + loss_single_point
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

        prec1, prec5 = accuracy(cls_logits.data.contiguous(), target, topk=(1, 2))
...
def val_loc_one_epoch(val_loader, model, device, criterion,epoch, writer, cfg, update_val_step):
    ...
    with torch.no_grad():
        ...
        for i, (input, target, bbox, image_names) in enumerate(val_loader):
            ...
            cls_logits, _ = model(input)
            loss = criterion(cls_logits, target)

            prec1, prec5 = accuracy(cls_logits.data, target, topk=(1, 2))
            losses.update(loss.item(), input.size(0))
            cls_top1.update(prec1.item(), input.size(0))
            cls_top5.update(prec5.item(), input.size(0))
    ...
    return update_val_step, losses.avg
