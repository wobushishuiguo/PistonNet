...
def accuracy(output, target, topk=(1,)):
    ...
    pred_enp = -torch.log2(output)
    pred_enp = pred_enp * output
    pred_enp = torch.sum(pred_enp, dim=1).unsqueeze(1) / 7
    output_pred = torch.cat((pred_enp, 1 - pred_enp), dim=1)
    _, pred = output_pred.topk(maxk, 1, True, True)
    pred = pred.t()
